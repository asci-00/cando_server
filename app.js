...
app.get("/equip/check/all", async (req, res) => {
  ...
  try {
    ...
    const select = await connection.query("SELECT * FROM Check_Log");
...
app.get("/equip/list_m", async (req, res) => {
  ...
  const connection = await DB.getConnection();
  const select = await connection.query("SELECT * FROM Equip_info;");
  const where = await connection.query("SELECT id FROM Equip_info WHERE = ?;", [
    serial,
  ]);
  const where1 = where[0][0]["id"];
  const search = await connection.query(
    "SELECT prs FROM Check_Log WHERE equip_id =? ORDER by id;",
    [where1]
  );
  //await connection.release.end();
  try {
...
app.post("/equip/pressure", async (req, res) => {
  ...
  try {
    ...
    const select = await connection.query(
      "SELECT * FROM Equip_info WHERE serial = ?;",
      [serial]
    );
    const result = await connection.query(
      "UPDATE Equip_info SET prs = ? WHERE serial = ?;",
      [pressure, serial]
    );
  ...
  try {
    ...
    const insert = await connection.query(
      ...
      [
        ...
        boarding_location,
        map,
        "https://chart.googleapis.com/chart?cht=qr&chs=200x200&chl=" + serial,
      ]
    );
    res.status(201).json({
      message: "Success",
...
app.get("/equip/check", async (req, res) => {
  ...
  const connection = await DB.getConnection();
  const select = await connection.query(
    "SELECT * FROM Check_Log WHERE equip_id =?;",
    [Eq_id]
  );
...
app.post("/equip/check/insert", async (req, res) => {
  ...
  try {
    ...
    const select = await connection.query(
      "INSERT INTO Check_Log(equip_id,date,check_res,user,prs) VALUES(?,now(),?,?,?);",
      [result, res_, user, pressure]
    );
    const update = await connection.query(
      "UPDATE Equip_info SET branch_check = 1 WHERE id =? ;",
      [result]
    );
    res.status(201).json({
      message: "Success",
...
app.get("/equip/checklist", async (req, res) => {
  ...
  try {
    ...
    const select = await connection.query("SELECT * FROM Check_List;");
    //await connection.release.end();
    let i = 0;
    let data = [];
    while (1) {
...
app.get("/map/list", async (req, res) => {
  ...
  const connection = await DB.getConnection();
  try {
    const select = await connection.query("SELECT * FROM Map;");
...
app.post("/upload/fet", upload.single("img"), async (req, res) => {
  ...
  try {
    ...
    const update = await connection.query(
      "UPDATE Equip_info SET image = ? WHERE serial = ?",
      [img, imgname]
    );
    res.status(201).json({
      message: "Success",
...
app.post("/upload/map", upload.single("img"), async (req, res) => {
  ...
  try {
    ...
    const update = await connection.query(
      "INSERT INTO Map(name, image) VALUES(?,?)",
      [map, img]
    );
    res.status(201).json({
      message: "Success",
...
app.post("/update/map", upload.single("img"), async (req, res) => {
  ...
  try {
    const update = await connection.query(
      "UPDATE Map SET image = ? WHERE name =?",
      [img, map]
    );
    res.status(201).json({
      message: "Success",
...
app.get("/download/map", async (req, res) => {
  ...
  const connection = await DB.getConnection();
  try {
    const search = await connection.query("SELECT * FROM Map WHERE name = ?;", [
      FN,
    ]);
    let path = search[0][0]["image"];
    res.sendFile(
      "/home/user/project/cando/public/imgs/" + path,
      function (err) {
        if (err) {
          console(err);
        } else res.status(201);
